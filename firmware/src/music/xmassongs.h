 ...
 * 
 *      Originally created for the Particle Christmas Tree Shield: 
 *       https://github.com/particle-iot/xmastree. Modified to add Particle
 *      Mesh publish for netowrked light displays.
 *
...
void playJoyToTheWorld()
{
  ...
  for (int i = 0; i < noteCount * 3; i += 3)
  {
    ...
    Mesh.publish("tone", String(notes_joyToTheWorld[i]));
    tone(BUZZER_PIN, notes_joyToTheWorld[i], notes_joyToTheWorld[i + 1]);
...
void playRudolfTheRedNosedReindeer()
{
  ...
  for (int i = 0; i < noteCount * 3; i += 3)
  {
    ...
    Mesh.publish("tone", String(notes_rudolfTheRedNosedReindeer[i]));
    tone(BUZZER_PIN, notes_rudolfTheRedNosedReindeer[i], notes_rudolfTheRedNosedReindeer[i + 1]);
...
void playJingleBells()
{
  ...
  for (int i = 0; i < noteCount * 3; i += 3)
  {
    ...
    Mesh.publish("tone", String(notes_jingleBells[i]));
    tone(BUZZER_PIN, notes_jingleBells[i], notes_jingleBells[i + 1]);
...
void playSilentNight()
{
  ...
  for (int i = 0; i < noteCount * 2; i += 2)
  {
    ...
    Mesh.publish("tone", String(notes_jingleBells[i]));
    tone(BUZZER_PIN, notes_jingleBells[i], notes_silentNight[i + 1]);
